...
def separate_sources(
    ...
    while start < length - overlap_frames:
        chunk = mix[:, :, start:end]
        with torch.no_grad():
            # print(f"forward pass in shape: {chunk.shape}")
            out = model.forward(chunk)
            # print(f"forward pass out shape: {out.shape}")
        out = fade(out)
        final[:, :, :, start:end] += out
        if start == 0:
...
sources = sources * ref.std() + ref.mean()      # un-normalization
...
def output_results(original_source: torch.Tensor, predicted_source: torch.Tensor, source: str):
    ...
    torchaudio.save(f"examples/demucs_test_{source}.wav", predicted_source, sample_rate)
...
# Mixture Clip
print(f"Mixed spectrogram size: {stft(mix_spec)[0].shape}")
plot_spectrogram(stft(mix_spec)[0], "Spectrogram - Mixture")
Audio(mix_spec, rate=sample_rate)
torchaudio.save(f"examples/demucs_test_mix.wav", mix_spec, sample_rate)
...
# Drums Clip
print(f"Drums spectrogram size: {drums_spec.shape}")
...
# Full Audio
Audio(mixture.cpu(), rate=sample_rate)

# Stems
for stem in audios.keys():
    Audio(audios[stem].cpu(), rate=sample_rate)
    torchaudio.save(f"examples/demucs_test_{stem}_FULL.wav", audios[stem].cpu(), sample_rate)
